...
@section('content')
	<div class="margin-top" style="max-width: 1000px; margin: 0 auto;">
		...
		<div class="d-flex align-items-start w-100 rounded shadow">
					...
					<div class="col-12 d-flex flex-column flex-lg-row col-12">
						<div class="col-4">
							@if (Str::startsWith($apartment->image, 'http'))
								<img class="object-fit-cover rounded shadow-sm my-2" src="{{ $apartment->image }}"
									alt="{{ $apartment->title }}" style="width: 15rem; height: 15rem; border-radius: 10px;">
							@else
								<img class="object-fit-cover rounded shadow-sm my-2" src="{{ asset('storage/' . $apartment->image) }}"
									style="width: 15rem; height: 15rem; border-radius: 10px;" alt="{{ $apartment->title }}">
							@endif
						</div>
						<div class="mb-3 col-12 col-sm-8 d-flex flex-column justify-content-between">
							...
							@error('image')
								<div class="text-danger">{{ $message }}</div>
							@enderror
							<p id="fileSizeError" style="color:red; display:none;">Il file supera le dimensioni massime di
								5 mb.</p>
							<div class="mb-3">
